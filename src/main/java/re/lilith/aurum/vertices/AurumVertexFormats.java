...
public class AurumVertexFormats {
    ...
    static {
        ...
        // This must start with the same elements, in the same order, as the vanilla entity format, because the
        // vanilla writers fill those elements and Aurum only appends to them.
        entityElements.add(VertexFormats.POSITION_ELEMENT); // 12
        entityElements.add(VertexFormats.TEXTURE_FLOAT_ELEMENT); // 20
        entityElements.add(VertexFormats.NORMAL_ELEMENT); // 23
        entityElements.add(VertexFormats.PADDING_ELEMENT); // 24
        entityElements.add(MID_TEXTURE_ELEMENT); // 32
        entityElements.add(TANGENT_ELEMENT); // 36
