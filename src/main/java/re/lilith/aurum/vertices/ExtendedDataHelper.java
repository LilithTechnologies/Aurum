...
public final class ExtendedDataHelper {
    ...
    public static int packMidBlock(float x, float y, float z, byte blockEmission) {
        return ((int) (x * 64) & 0xFF) | (((int) (y * 64) & 0xFF) << 8) | (((int) (z * 64) & 0xFF) << 16)
                | ((blockEmission & 0xFF) << 24);
    }

    public static int computeMidBlock(float x, float y, float z, int localPosX, int localPosY, int localPosZ, byte blockEmission) {
        return packMidBlock(
                localPosX + 0.5f - x,
                localPosY + 0.5f - y,
                localPosZ + 0.5f - z,
