...
import re.lilith.aurum.expression.cached.CachedUniform;
import re.lilith.aurum.gl.uniform.holder.LocationalUniformHolder;
...
// Refactoring it is on the roadmap
public class CustomUniforms implements FunctionContext {
    ...
    private CustomUniforms(CustomUniformFixedInputUniformsHolder inputHolder, Map<String, Builder.Variable> variables) {
        ...
        for (Builder.Variable variable : variables.values()) {
            try {
                ...
                // TODO: fix custom uniforms
                // Aurum.LOGGER.warn("Failed to resolve uniform {}, reason: {} ( = {})", variable.name, e.getMessage(), variable.expression, e);
            }
        }
    ...
    public static class Builder {
        final private static Map<String, Type> types = new ImmutableMap.Builder<String, Type>()
                ...
                .put("mat3", MatrixType.MAT3)
                .put("mat4", MatrixType.MAT4)
                .build();
        ...
        public void addVariable(String type, String name, String expression, boolean isUniform) {
            if (variables.containsKey(name)) {
                Aurum.LOGGER.warn("Ignoring duplicated custom uniform name: " + name);
                return;
            }

            Type parsedType = types.get(type);
            if (parsedType == null) {
                Aurum.LOGGER.warn("Ignoring invalid uniform type: " + type + " of " + name);
                return;
            }

            try {
                ExpressionElement ast = Parser.parse(expression, AurumOptions.options);
                variables.put(name, new Variable(parsedType, name, ast, isUniform));
            } catch (Exception e) {
                Aurum.LOGGER.warn("Failed to parse custom variable/uniform " + name + " with expression " + expression, e);
            }
        }
