...
public final class CelestialUniforms {
    ...
    public void addCelestialUniforms(UniformHolder uniforms) {
        uniforms
                ...
                .uniformTruncated3f(PER_FRAME, "endFlashPosition", () -> ZERO_VECTOR_4f);
    }
    ...
    private Vector4f getCelestialPositionInWorldSpace(float y) {
        ...
        celestial.rotateY((float) toRadians(-90.0F));
        celestial.rotateZ((float) toRadians(sunPathRotation));
        celestial.rotateX((float) toRadians(getSkyAngle() * 360.0F));
    ...
    private static Vector4f getUpPosition() {
        ...
        preCelestial.rotateY((float) toRadians(-90.0F));
