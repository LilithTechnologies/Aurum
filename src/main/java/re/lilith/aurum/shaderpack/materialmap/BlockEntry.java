    ...
    public static BlockEntry parse(@NotNull String entry) {
        ...
        for (int index = statesStart; index < splitStates.length; index++) {
            ...
            if (propertyParts.length != 2) {
                Aurum.LOGGER.warn("Warning: the block ID map entry \"{}\" could not be fully parsed:", entry);
                Aurum.LOGGER.warn("- Block state property filters must be of the form \"key=value\", but {} is not of that form!", splitStates[index]);
