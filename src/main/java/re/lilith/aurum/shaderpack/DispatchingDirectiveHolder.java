...
import re.lilith.aurum.AurumFlags;
...
public class DispatchingDirectiveHolder implements DirectiveHolder {
        ...
        if (type == ConstDirectiveParser.Type.BOOL) {
            ...
            if (consumer != null) {
                if ("true".equals(value)) {
                    ...
                    Aurum.LOGGER.error("Failed to process " + directive + ": " + value + " is not a valid boolean value");
                }
            ...
            if (AurumFlags.DEBUG) {
                // Only logspam in dev
                Aurum.LOGGER.info("Found potential unhandled directive: " + directive);
            }
            ...
            if (intConsumer != null) {
                try {
                    ...
                    Aurum.LOGGER.error("Failed to process " + directive, e);
                }
            ...
            if (consumer != null) {
                if (!value.startsWith("vec2")) {
                    Aurum.LOGGER.error("Failed to process " + directive + ": value was not a valid vec2 constructor");
                }
                ...
                if (parts.length != 2) {
                    Aurum.LOGGER.error("Failed to process " + directive +
            ...
            if (consumer != null) {
                if (!value.startsWith("ivec3")) {
                    Aurum.LOGGER.error("Failed to process " + directive + ": value was not a valid ivec3 constructor");
                }
            ...
            if (consumer != null) {
                if (!value.startsWith("vec4")) {
                    Aurum.LOGGER.error("Failed to process " + directive + ": value was not a valid vec4 constructor");
                }
                ...
                if (parts.length != 4) {
                    Aurum.LOGGER.error("Failed to process {}: expected 4 arguments to a vec4 constructor, got {}", directive, parts.length);
                }
    ...
    private void typeCheckHelper(String expected, Map<String, ? extends Object> candidates, ConstDirectiveParser.ConstDirective directive) {
        if (candidates.containsKey(directive.getKey())) {
            Aurum.LOGGER.warn("Ignoring {} because it is of the wrong type, a type of {} is expected.", directive, expected);
        }
    }
    ...
    public void acceptUniformDirective(String name, Runnable onDetected) {
        if (AurumFlags.DEBUG) {
            Aurum.LOGGER.warn("Not looking for a uniform directive with the name {} since this type of directive is not currently supported.", name);
        }
    }
    ...
    public void acceptCommentStringDirective(String name, Consumer<String> consumer) {
        if (AurumFlags.DEBUG) {
            Aurum.LOGGER.warn("Not looking for a comment string directive with the name {} since this type of directive is not currently supported.", name);
        }
    }
    ...
    public void acceptCommentIntDirective(String name, IntConsumer consumer) {
        if (AurumFlags.DEBUG) {
            Aurum.LOGGER.warn("Not looking for a comment int directive with the name {} since this type of directive is not currently supported.", name);
        }
    }
    ...
    public void acceptCommentFloatDirective(String name, FloatConsumer consumer) {
        if (AurumFlags.DEBUG) {
            Aurum.LOGGER.warn("Not looking for a comment float directive with the name {} since this type of directive is not currently supported.", name);
        }
    }
