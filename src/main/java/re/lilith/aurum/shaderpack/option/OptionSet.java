    ...
    public static class Builder {
        ...
        public void addBooleanOption(MergedBooleanOption proposed) {
            ...
            if (existing != null) {
                ...
                if (merged == null) {
                    Aurum.LOGGER.warn("Ignoring ambiguous boolean option " + option.getName()
                            + ", conflicting default values declared at: "
                            + describeLocations(existing.getLocations(), proposed.getLocations()));
        ...
        public void addStringOption(MergedStringOption proposed) {
            ...
            if (existing != null) {
                ...
                if (merged == null) {
                    Aurum.LOGGER.warn("Ignoring ambiguous string option " + option.getName()
                            + ", conflicting default values declared at: "
                            + describeLocations(existing.getLocations(), proposed.getLocations()));
        ...
        private static String describeLocations(Set<OptionLocation> a, Set<OptionLocation> b) {
            return Stream.concat(a.stream(), b.stream())
                    .map(location -> location.getFilePath() + ":" + (location.getLineIndex() + 1))
                    .distinct()
