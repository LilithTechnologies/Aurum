...
import re.lilith.aurum.shaderpack.parsing.ParsedString;
...
public final class OptionAnnotatedSource {
     ...
     * author may have intended for a line to be a valid option, Aurum might have ignored it due to
     * a syntax error or some other issue.
     * <p>
     * These diagnostic messages provide reasons for why Aurum decided to ignore a plausible option
    ...
    public OptionAnnotatedSource(final ImmutableList<String> lines) {
        ...
        ImmutableMap.Builder<String, IntList> booleanDefineReferences = ImmutableMap.builder();
        builder.booleanDefineReferences.forEach((name, indices) ->
                booleanDefineReferences.put(name, IntLists.unmodifiable(indices)));
        this.booleanDefineReferences = booleanDefineReferences.build();
    }
