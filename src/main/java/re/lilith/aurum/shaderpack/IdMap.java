...
public class IdMap {
    ...
    private static Optional<Properties> loadProperties(Path shaderPath, String name, ShaderPackOptions shaderPackOptions,
        ...
        try {
            ...
            Aurum.LOGGER.error("Error loading " + name + " at " + shaderPath, e);
    ...
    private static Object2IntMap<NamespacedId> parseIdMap(Properties properties, String keyPrefix, String fileName) {
        ...
        properties.forEach((keyObject, valueObject) -> {
            ...
            try {
                ...
                Aurum.LOGGER.warn("Failed to parse line in " + fileName + ": invalid key " + key);
                return;
            }
            ...
            for (String part : value.split("\\s+")) {
                if (part.contains("=")) {
                    ...
                    Aurum.LOGGER.warn("Failed to parse an ResourceLocation in " + fileName + " for the key " + key + ": state properties are currently not supported: " + part);
                    continue;
                }
        ...
        properties.forEach((keyObject, valueObject) -> {
            ...
            for (String part : value.split("\\s+")) {
                ...
                try {
                    ...
                    Aurum.LOGGER.warn("Unexpected error while parsing an entry from " + fileName + " for the key " + key + ":", e);
                }
            }
    ...
    private static Map<NamespacedId, BlockRenderType> parseRenderTypeMap(Properties properties, String keyPrefix, String fileName) {
        ...
        properties.forEach((keyObject, valueObject) -> {
            ...
            if (renderType == null) {
                Aurum.LOGGER.warn("Failed to parse line in " + fileName + ": invalid block render type: " + key);
                return;
            }
