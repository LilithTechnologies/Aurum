...
public class PackShadowDirectives {
    public static final int MAX_SHADOW_COLOR_BUFFERS_BASELINE = 2;
    public static final int MAX_SHADOW_COLOR_BUFFERS_EXTENDED = 8;
    ...
    public PackShadowDirectives(ShaderProperties properties) {
        ...
        //
        // However, Aurum does still does cull shadows whenever it can - but, it does so by analyzing
        ...
        int maxShadowColorBuffers = wantsHigherShadowColor(properties)
                ? MAX_SHADOW_COLOR_BUFFERS_EXTENDED
                : MAX_SHADOW_COLOR_BUFFERS_BASELINE;

        for (int i = 0; i < maxShadowColorBuffers; i++) {
    ...
    private static boolean wantsHigherShadowColor(ShaderProperties properties) {
        return properties.getRequiredFeatureFlags().stream().anyMatch(flag -> FeatureFlags.getValue(flag) == FeatureFlags.HIGHER_SHADOWCOLOR)
                || properties.getOptionalFeatureFlags().stream().anyMatch(flag -> FeatureFlags.getValue(flag) == FeatureFlags.HIGHER_SHADOWCOLOR);
    }
    ...
    public boolean isDistanceRenderImplicit() {
        ...
        for (int i = 0; i < settings.size(); i++) {
            ...
            directives.acceptConstStringDirective(bufferName + "Format", format -> {
                ...
                if (internalFormat.isPresent()) {
                    ...
                    Aurum.LOGGER.warn("Unrecognized internal texture format {} specified for {}Format, ignoring.", format, bufferName);
                }
            });
