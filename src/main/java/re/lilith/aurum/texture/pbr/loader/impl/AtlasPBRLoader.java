package re.lilith.aurum.texture.pbr.loader.impl;
...
import re.lilith.aurum.mixin.texture.AnimationMetadataAccessor;
import re.lilith.aurum.mixin.texture.SpriteAccessor;
import re.lilith.aurum.mixin.texture.SpriteAtlasTextureAccessor;
...
import re.lilith.aurum.texture.pbr.PBRAtlasTexture;
...
import re.lilith.aurum.texture.util.ImageManipulationUtil;
...
public class AtlasPBRLoader implements PBRTextureLoader<SpriteAtlasTexture> {
    ...
    public void load(SpriteAtlasTexture atlas, ResourceManager resourceManager, PBRTextureConsumer pbrTextureConsumer) {
        ...
        for (Sprite sprite : ((SpriteAtlasTextureAccessor) atlas).getTexturesByName().values()) {
            if (!sprite.getName().equals(SpriteAtlasTexture.MISSING.toString())) {
                ...
                if (normalSprite != null) {
                    ...
                    PBRSpriteHolder pbrSpriteHolder = ((TextureAtlasSpriteExtension) sprite).aurum$getOrCreatePBRHolder();
    ...
    protected static int fetchAtlasMipLevel(SpriteAtlasTexture atlas) {
        ...
        return ((SpriteAccessor) missingSprite).getMainImage().length - 1;
    }
    ...
    protected Sprite createPBRSprite(Sprite sprite, ResourceManager resourceManager, SpriteAtlasTexture atlas, int atlasWidth, int atlasHeight, int mipLevel, PBRType pbrType) {
        ...
        Identifier imageLocation = ((SpriteAtlasTextureAccessor) atlas).completeResourceLocation(spriteName, 0);
        ...
        try {
            ...
            if (frameWidth != targetFrameWidth || frameHeight != targetFrameHeight) {
                ...
                if (!animationMetadata.equals(EMPTY_ANIMATION_METADATA)) {
                    AnimationMetadataAccessor animationAccessor = (AnimationMetadataAccessor) animationMetadata;
            ...
            SpriteInfo pbrSpriteInfo = new PBRTextureAtlasSpriteInfo(pbrSpriteName, frameWidth, frameHeight, pbrType);

            int x = ((SpriteAccessor) sprite).getX();
            int y = ((SpriteAccessor) sprite).getY();
            ...
            Aurum.LOGGER.error("Unable to parse metadata from {} : {}", pbrImageLocation, e);
        } catch (IOException e) {
            Aurum.LOGGER.error("Unable to load {} : {}", pbrImageLocation, e);
        }
    ...
    protected void syncAnimation(Sprite source, Sprite target) {
        ...
        SpriteAccessor sourceAccessor = ((SpriteAccessor) source);
        ...
        SpriteAccessor targetAccessor = ((SpriteAccessor) target);
    ...
    protected static class PBRTextureAtlasSpriteInfo extends SpriteInfo {
        ...
        public PBRTextureAtlasSpriteInfo(Identifier name, int width, int height, PBRType pbrType) {
    ...
    public static class PBRTextureAtlasSprite extends Sprite implements CustomMipmapGenerator.Provider {
        protected PBRTextureAtlasSprite(SpriteInfo info, int mipLevel, int atlasWidth, int atlasHeight, int x, int y, NativeImage nativeImage) {
            ...
            CustomMipmapGenerator generator = getMipmapGenerator(info, atlasWidth, atlasHeight);
            assert generator != null;
            NativeImage[] mipImages = generator.generateMipLevels(nativeImage, mipLevel);

            int[][] mainImage = new int[mipImages.length][];
            for (int level = 0; level < mipImages.length; level++) {
                mainImage[level] = mipImages[level].makePixelArray();
                mipImages[level].close();
            }

            ((SpriteAccessor) this).setMainImage(mainImage);
        }
