...
public interface TextureFormat {
    ...
    default void setupTextureParameters(PBRType pbrType, AbstractTexture texture) {
        if (!canInterpolateValues(pbrType)) {
            int minFilter = AurumRenderSystem.getTexParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER);
            ...
            AurumRenderSystem.texParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, mipmap ? GL11.GL_NEAREST_MIPMAP_NEAREST : GL11.GL_NEAREST);
            AurumRenderSystem.texParameteri(texture.getGlId(), GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_NEAREST);
        }
    }
