...
import re.lilith.aurum.targets.depth.DepthTexture;
...
public class RenderTargets {
    ...
    public void copyPreTranslucentDepth() {
        if (translucentDepthDirty) {
            ...
            AurumRenderSystem.copyTexImage2D(GL11.GL_TEXTURE_2D, 0, currentDepthFormat.getGlInternalFormat(), 0, 0, cachedWidth, cachedHeight, 0);
        } else {
    ...
    public GlFramebuffer createFramebufferWritingToMain(int[] drawBuffers) {
        return createFullFramebuffer(drawBuffers);
    }
    ...
    private GlFramebuffer createFullFramebuffer(int[] drawBuffers) {
        if (drawBuffers.length == 0) {
            return createEmptyFramebuffer();
        }

        return createColorFramebufferWithDepth(invert(ImmutableSet.of(), drawBuffers), drawBuffers);
    }
