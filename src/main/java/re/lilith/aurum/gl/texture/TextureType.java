...
public enum TextureType {
    ...
    public void apply(int texture, int sizeX, int sizeY, int sizeZ, int internalFormat, int format, int pixelType, ByteBuffer pixels) {
        switch (this) {
            case TEXTURE_1D ->
                    AurumRenderSystem.texImage1D(texture, glType, 0, internalFormat, sizeX, 0, format, pixelType, pixels);
            case TEXTURE_2D, TEXTURE_RECTANGLE ->
                    AurumRenderSystem.texImage2D(texture, glType, 0, internalFormat, sizeX, sizeY, 0, format, pixelType, pixels);
            case TEXTURE_3D ->
                    AurumRenderSystem.texImage3D(texture, glType, 0, internalFormat, sizeX, sizeY, sizeZ, 0, format, pixelType, pixels);
        }
    }
}
