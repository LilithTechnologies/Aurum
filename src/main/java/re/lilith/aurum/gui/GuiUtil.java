...
public final class GuiUtil {
    public static final Identifier AURUM_WIDGETS_TEX = new Identifier("aurum", "textures/gui/widgets.png");
    ...
    /**
     * Binds Aurum's widgets texture to be
     * used for succeeding draw calls.
     */
    public static void bindAurumWidgetsTexture() {
        client().getTextureManager().bindTexture(AURUM_WIDGETS_TEX);
    }
    ...
    public static void drawButton(int x, int y, int width, int height, boolean hovered, boolean disabled) {
        ...
        // Sets GlStateManager to use solid white as the tint color for blend mode, and enables blend mode
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        GlStateManager.enableBlend();

        // Sets GlStateManager to be able to use textures when drawing
    ...
    public static void drawTexture(int x, int y, int u, int v, int width, int height) {
        ...
        bufferBuilder.vertex(x, y + height, 0.0D).texture((float) (u) * f, (float) (v + height) * g).next();
        bufferBuilder.vertex(x + width, y + height, 0.0D).texture((float) (u + width) * f, (float) (v + height) * g).next();
        bufferBuilder.vertex(x + width, y, 0.0D).texture((float) (u + width) * f, (float) (v) * g).next();
        bufferBuilder.vertex(x, y, 0.0D).texture((float) (u) * f, (float) (v) * g).next();
        tessellator.draw();
    }
    ...
    public static Text translateOrDefault(Text defaultText, String translationDesc, Object... format) {
        String translated = I18n.translate(translationDesc, format);

        if (translated.equals(translationDesc)) {
            return defaultText;
        }

        return new LiteralText(translated);
    }
