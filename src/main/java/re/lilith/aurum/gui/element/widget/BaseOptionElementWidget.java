...
public abstract class BaseOptionElementWidget<T extends OptionMenuElement> extends CommentedElementWidget<T> {
    protected static final Text SET_TO_DEFAULT = new TranslatableText("options.aurum.setToDefault").setStyle(new Style().setFormatting(Formatting.GREEN));
    ...
    public Optional<Text> getCommentBody() {
        return Optional.ofNullable(getCommentKey()).map(key -> new LiteralText(I18n.translate(key)));
    }
