...
public class ShaderPackSelectionList extends AurumObjectSelectionList {
    private static final Text PACK_LIST_LABEL = new TranslatableText("pack.aurum.list.label").setStyle(new Style().setItalic(true).setFormatting(Formatting.GRAY));
    ...
    public ShaderPackSelectionList(ShaderPackScreen screen, MinecraftClient client, int width, int height, int top, int bottom, int left, int right) {
        ...
        this.screen = screen;
        this.topButtonRow = new TopButtonRowEntry(this, Aurum.getAurumConfig().areShadersEnabled());
    ...
    public void refresh() {
        ...
        try {
            names = Aurum.getShaderpacksDirectoryManager().enumerate();
        } catch (Throwable e) {
            Aurum.LOGGER.error("Error reading files while constructing selection UI", e);
    ...
    public void addPackEntry(String name) {
        ShaderPackEntry entry = new ShaderPackEntry(this, name);

        Aurum.getAurumConfig().getShaderPackName().ifPresent(currentPackName -> {
    ...
    public static class TopButtonRowEntry extends BaseEntry {
        private static final Text REFRESH_SHADER_PACKS_LABEL = new TranslatableText("options.aurum.refreshShaderPacks").setStyle(new Style().setFormatting(Formatting.DARK_PURPLE));
        private static final Text NONE_PRESENT_LABEL = new TranslatableText("options.aurum.shaders.nonePresent").setStyle(new Style().setFormatting(Formatting.GRAY));
        private static final Text SHADERS_DISABLED_LABEL = new TranslatableText("options.aurum.shaders.disabled");
        private static final Text SHADERS_ENABLED_LABEL = new TranslatableText("options.aurum.shaders.enabled");
        private static final int REFRESH_BUTTON_WIDTH = 18;

        private final ShaderPackSelectionList list;
        private final AurumElementRow buttons = new AurumElementRow();
        private final EnableShadersButtonElement enableDisableButton;
        private final AurumElementRow.Element refreshPacksButton;
        ...
        public TopButtonRowEntry(ShaderPackSelectionList list, boolean shadersEnabled) {
            ...
            this.refreshPacksButton = new AurumElementRow.IconButtonElement(
        ...
        public static class EnableShadersButtonElement extends AurumElementRow.TextButtonElement {
            private int centerX;

            public EnableShadersButtonElement(Text text, Function<AurumElementRow.TextButtonElement, Boolean> onClick) {
                super(text, onClick);
            }
