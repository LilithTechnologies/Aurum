package re.lilith.aurum.gui.screen;
...
import re.lilith.aurum.gui.element.ShaderPackSelectionList;
import re.lilith.aurum.gui.element.widget.AbstractElementWidget;
import re.lilith.aurum.gui.element.widget.AurumButtonWidget;
import re.lilith.aurum.gui.element.widget.AurumClickableImageWidget;
import re.lilith.aurum.gui.element.widget.CommentedElementWidget;
...
public class ShaderPackScreen extends Screen implements HudHideable {
    ...
    private static final Text SELECT_TITLE = new TranslatableText("pack.aurum.select.title").setStyle(new Style().setFormatting(Formatting.GRAY).setItalic(true));
    private static final Text CONFIGURE_TITLE = new TranslatableText("pack.aurum.configure.title").setStyle(new Style().setFormatting(Formatting.GRAY).setItalic(true));
    private static final int COMMENT_PANEL_WIDTH = 314;

    private final Screen parent;
    private final Text aurumTextComponent;
    ...
    public ShaderPackScreen(Screen parent) {
        ...
        String aurumName = Aurum.MODNAME + " " + Aurum.getVersion();

        if (aurumName.contains("-development-environment")) {
            this.developmentComponent = new LiteralText("Development Environment").setStyle(new Style().setFormatting(Formatting.GOLD));
            aurumName = aurumName.replace("-development-environment", "");
        }

        this.aurumTextComponent = new LiteralText(aurumName).setStyle(new Style().setFormatting(Formatting.GRAY));
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        if (!this.guiHidden) {
            drawCenteredString(this.textRenderer, "Aurum", (int) (this.width * 0.5), 8, 0xFFFFFF);
        ...
        if (this.developmentComponent != null) {
            ...
            this.textRenderer.drawWithShadow(aurumTextComponent.asFormattedString(), 2, this.height - 20, 0xFFFFFF);
            ...
            this.textRenderer.drawWithShadow(aurumTextComponent.asFormattedString(), 2, this.height - 10, 0xFFFFFF);
        }
    }
    ...
    public void init() {
        ...
        if (Aurum.getCurrentPack().isPresent() && this.navigation != null) {
            ShaderPack currentPack = Aurum.getCurrentPack().get();
        ...
        if (!this.guiHidden) {
            this.buttons.add(new AurumButtonWidget(bottomCenter + 104, this.height - 27, 100, 20,
                    I18n.translate("gui.done"), button -> this.client.setScreen(parent)));

            this.buttons.add(new AurumButtonWidget(bottomCenter, this.height - 27, 100, 20,
                    I18n.translate("options.aurum.apply"), button -> this.applyChanges()));

            this.buttons.add(new AurumButtonWidget(bottomCenter - 104, this.height - 27, 100, 20,
                    I18n.translate("gui.cancel"), button -> this.dropChangesAndClose()));

            this.buttons.add(new AurumButtonWidget(topCenter - 78, this.height - 51, 152, 20,
                    I18n.translate("options.aurum.openShaderPackFolder"), button -> openShaderPackFolder()));

            this.screenSwitchButton = new AurumButtonWidget(topCenter + 78, this.height - 51, 152, 20,
                    I18n.translate("options.aurum.shaderPackList"), button -> {
        ...
        if (inWorld) {
            Text showOrHide = this.guiHidden
                    ? new TranslatableText("options.aurum.gui.show")
                    : new TranslatableText("options.aurum.gui.hide");
            ...
            this.buttons.add(new AurumClickableImageWidget(
                    x, this.height - 39,
                    20, 20,
                    this.guiHidden ? 20 : 0, 146, 20,
                    GuiUtil.AURUM_WIDGETS_TEX,
                    button -> {
    ...
    public void refreshScreenSwitchButton() {
        if (this.screenSwitchButton != null) {
            this.screenSwitchButton.message = (
                    optionMenuOpen ?
                            new TranslatableText("options.aurum.shaderPackList")
                            : new TranslatableText("options.aurum.shaderPackSettings")
            ).asFormattedString();
    ...
    public void onPackListFilesDrop(List<Path> paths) {
        List<Path> packs = paths.stream().filter(Aurum::isValidShaderpack).collect(Collectors.toList());

        for (Path pack : packs) {
            String fileName = pack.getFileName().toString();

            try {
                Aurum.getShaderpacksDirectoryManager().copyPackIntoDirectory(fileName, pack);
            } catch (FileAlreadyExistsException e) {
                this.notificationDialog = new TranslatableText(
                        "options.aurum.shaderPackSelection.copyErrorAlreadyExists",
                        fileName
                ...
                Aurum.LOGGER.warn("Error copying dragged shader pack", e);

                this.notificationDialog = new TranslatableText(
                        "options.aurum.shaderPackSelection.copyError",
                        fileName
                ).setStyle(new Style().setFormatting(Formatting.RED).setItalic(true));
        ...
        if (packs.size() == 0) {
            ...
            if (paths.size() == 1) {
                ...
                this.notificationDialog = new TranslatableText(
                        "options.aurum.shaderPackSelection.failedAddSingle",
                        fileName
                ).setStyle(new Style().setFormatting(Formatting.RED).setItalic(true));
            } else {
                ...
                this.notificationDialog = new TranslatableText(
                        "options.aurum.shaderPackSelection.failedAdd"
                ).setStyle(new Style().setFormatting(Formatting.RED).setItalic(true));
            }
            ...
            this.notificationDialog = new TranslatableText(
                    "options.aurum.shaderPackSelection.addedPack",
                    packName
            ...
            // in that case.
            this.notificationDialog = new TranslatableText(
                    "options.aurum.shaderPackSelection.addedPacks",
                    packs.size()
            ).setStyle(new Style().setFormatting(Formatting.YELLOW).setItalic(true));
        }
    ...
    public void onOptionMenuFilesDrop(List<Path> paths) {
        ...
        if (paths.size() != 1) {
            this.notificationDialog = new TranslatableText(
                    "options.aurum.shaderPackOptions.tooManyFiles"
            ).setStyle(new Style().setFormatting(Formatting.RED).setItalic(true));
    ...
    public void importPackOptions(Path settingFile) {
        try (InputStream in = Files.newInputStream(settingFile)) {
            ...
            Aurum.queueShaderPackOptionsFromProperties(properties);

            this.notificationDialog = new TranslatableText(
                    "options.aurum.shaderPackOptions.importedSettings",
            ...
            Aurum.LOGGER.error("Error importing shader settings file \"" + settingFile.toString() + "\"", e);

            this.notificationDialog = new TranslatableText(
                    "options.aurum.shaderPackOptions.failedImport",
                    settingFile.getFileName().toString()
            ).setStyle(new Style().setFormatting(Formatting.RED).setItalic(true));
            this.notificationDialogTimer = 100; // 5 seconds (100 ticks)
        }
    ...
    public void applyChanges() {
        ...
        if (!name.equals(Aurum.getCurrentPackName())) {
            Aurum.clearShaderPackOptionQueue();
        }

        final boolean enabled = this.shaderPackList.getTopButtonRow().shadersEnabled;

        final String previousPackName = Aurum.getAurumConfig().getShaderPackName().orElse(null);
        final boolean previousShadersEnabled = Aurum.getAurumConfig().areShadersEnabled();
        ...
        if (!name.equals(previousPackName) || enabled != previousShadersEnabled || !Aurum.getShaderPackOptionQueue().isEmpty() || Aurum.shouldResetShaderPackOptionsOnNextReload()) {
            Aurum.getAurumConfig().setShaderPackName(name);
            AurumConfig config = Aurum.getAurumConfig();
            ...
            try {
                ...
                Aurum.LOGGER.error("Error saving configuration file!", e);
            }

            try {
                Aurum.reload();
            } catch (IOException e) {
                Aurum.LOGGER.error("Error reloading shader pack while applying changes!", e);
            }
        }
    ...
    private void openShaderPackFolder() {
        Sys.openURL(Aurum.getShaderpacksDirectoryManager().root().toUri().toString());
    }
    ...
    protected void buttonClicked(ButtonWidget button) {
        if (button instanceof AurumButtonWidget ibw) {
            ibw.click();
            return;
        }
